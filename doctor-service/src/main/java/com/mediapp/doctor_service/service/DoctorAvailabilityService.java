...
public class DoctorAvailabilityService {
    ...
    public Mono<List<DoctorProfileResponse>> findDoctorsBySpecialty(Integer specialtyId) {
        ...
        return specialtyMono.flatMapMany(specialty -> doctorProfileRepository.findBySpecialtyId(specialtyId)
                ...
                .collectList()
                .doFinally(
                        signal -> stopTimer(sample, "doctor.availability.search.latency", "specialtyId", specialtyTag));
    }

    public Mono<List<AvailabilitySlotResponse>> getAvailability(UUID doctorId, Instant from, Instant to) {
        Timer.Sample sample = Timer.start(meterRegistry);
        ...
        return ensureDoctorExists(doctorId)
                .thenMany(availabilitySlotRepository
                        .findByDoctorIdAndStartTimeBetweenOrderByStartTimeAsc(doctorId, effectiveFrom, effectiveTo))
    ...
    public Mono<List<AvailabilitySlotResponse>> createRecurringSlots(UUID doctorId, CreateAvailabilityRequest request) {
        Timer.Sample sample = Timer.start(meterRegistry);
        ...
        return ensureDoctorExists(doctorId)
                .flatMap(doctor -> transactionalOperator.transactional(Mono.defer(() -> {
                    ...
                    return Flux.fromIterable(generated)
                            ...
                            .collectList()
                            .flatMap(slots -> availabilitySlotRepository.saveAll(slots).collectList());
                })))
                .map(slots -> slots.stream()
                        .map(doctorMapper::toAvailabilitySlotResponse)
                        .collect(Collectors.toList()))
                .doOnSuccess(list -> meterRegistry.counter("doctor.availability.create", "outcome", "success")
                        .increment())
                .doOnError(error -> meterRegistry.counter("doctor.availability.create", "outcome", "failed")
                        .increment())
                .doFinally(signal -> stopTimer(sample, "doctor.availability.create.latency", "doctorId",
                        doctorId.toString()));
    }

    public Mono<SlotReservationResponse> reserveSlot(UUID slotId, String reservationToken) {
        Timer.Sample sample = Timer.start(meterRegistry);
        ...
        return availabilitySlotRepository.findById(slotId)
                ...
                .flatMap(existing -> availabilitySlotRepository.reserveSlot(slotId, reservationToken, now)
                        .switchIfEmpty(Mono.error(new AvailabilityReservationConflictException(slotId))))
                .map(doctorMapper::toReservationResponse)
                .doOnSuccess(res -> meterRegistry.counter("doctor.availability.reserve", "outcome", "success")
                        .increment())
                .doOnError(error -> {
                    if (error instanceof AvailabilityReservationConflictException) {
                        meterRegistry.counter("doctor.availability.reserve", "outcome", "conflict").increment();
                    } else if (error instanceof AvailabilitySlotNotFoundException) {
                        meterRegistry.counter("doctor.availability.reserve", "outcome", "not_found").increment();
                    } else {
                        meterRegistry.counter("doctor.availability.reserve", "outcome", "failed").increment();
                    }
                })
    ...
    private Mono<DoctorProfileEntity> ensureDoctorExists(UUID doctorId) {
        return doctorProfileRepository.findById(doctorId)
                .switchIfEmpty(Mono.error(new DoctorNotFoundException(doctorId)));
    }

    private Mono<AvailabilitySlotEntity> ensureNoOverlap(UUID doctorId, AvailabilitySlotEntity slot) {
        return availabilitySlotRepository
                ...
                .flatMap(overlap -> overlap
                        ? Mono.<AvailabilitySlotEntity>error(
                                new AvailabilityOverlapException(doctorId, slot.getStartTime(), slot.getEndTime()))
                        : Mono.just(slot));
    }
